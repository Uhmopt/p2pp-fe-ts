...
import { Box, Collapse, Grid, GridDirection, GridSize, GridSpacing, GridWrap, Theme } from "@mui/material";
...
type EditFormProps<T> = PropsWithChildren<{
	...
	justifyContent?:
		...
		| ((theme: Theme) => ResponsiveStyleValue<string[] | FlexProperty | undefined>);
	alignItems?:
		| ResponsiveStyleValue<string[] | FlexProperty | undefined>
		| ((theme: Theme) => ResponsiveStyleValue<string[] | FlexProperty | undefined>);
	lg?: boolean | GridSize;
...
const EditForm = function <T = { [key: string]: any }>(
	...
	const [isValidFields, setIsValidFields] = useState<{ [key: string]: boolean }>({});

	const checkFormValid = () => {
		const newIsValidFields = formatArray<StaticField>(fields).reduce((ret, field, fieldIndex, self) => {
		...
		const isValid = Object.values(newIsValidFields).reduce((ret, cur) => ret && cur, true);

		return isValid && data ? data : false;
	};

	useImperativeHandle(ref, () => ({ prepare: checkFormValid }), [checkFormValid]);
	...
	return (
		<Grid container spacing={spacing} justifyContent={justifyContent} alignItems={alignItems} direction={direction} flexWrap={flexWrap}>
			{fields.map((field, fieldIndex) => {
				...
				return (
					...
					>
						<Collapse
							in={typeof field?.isHide === "boolean" ? !field?.isHide : typeof field?.isHide === "function" ? !field?.isHide(data) : true}
						>
							<Box>
								<EditFormControl
									data={data}
									...
									// eslint-disable-next-line @typescript-eslint/ban-ts-comment
									// @ts-ignore
									readOnly={readOnly || (field.readOnlyEdit && !!data.id && !(data.id === "new"))}
									isLabel={field?.isLabel ?? isLabel}
...
export default forwardRef(EditForm) as <T>(props: EditFormProps<T> & { ref?: ForwardedRef<{ prepare: () => boolean | T }> }) => JSX.Element;
